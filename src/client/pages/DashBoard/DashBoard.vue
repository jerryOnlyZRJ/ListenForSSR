    ...
    <div class="mui-content">
      <div class="mui-input-row mui-search">
        <input
          ...
          @input="filterWordList"
          placeholder="您可以搜索已存在的文档~"
        >
      </div>
      <div v-if="withoutWordItem" class="empty-info">
        <span>文档列表空空如也,</span>
        <span>快点击右上角添加文档吧~</span>
      </div>
  ...
  methods: {
    ...
    deleteThisWord(index, e) {
      ...
      function postAdeleteRequest(callback, deleteOptions) {
        request
          .post("/user/deleteword", deleteOptions)
          .then(res => {
            if (res.data.n) {
              callback();
            }
      ...
      if (this.filterList === this.orginList) {
        ...
        const indexInOriginList = this.orginList.indexOf(
          this.orginList.find(originItem => originItem.title === title)
        );
        postAdeleteRequest(() => {
          mui.toast(`删除${title}成功`);
          this.filterList.splice(index, 1);
